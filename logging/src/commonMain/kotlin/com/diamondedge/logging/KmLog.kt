...
open class KmLog(tag: String) {
    ...
    fun warn(err: Throwable?, tag: String? = null, msg: () -> Any?) {
        if (KmLogging.isLoggingWarning)
            warnApi(tag ?: tagName, msg().toString(), err)
    }

    fun w(err: Throwable? = null, tag: String? = null, msg: () -> Any?) {
        if (KmLogging.isLoggingWarning)
            warnApi(tag ?: tagName, msg().toString(), err)
    }
    ...
    fun error(err: Throwable?, tag: String? = null, msg: () -> Any?) {
        if (KmLogging.isLoggingError)
            errorApi(tag ?: tagName, msg().toString(), err)
    }

    fun e(err: Throwable? = null, tag: String? = null, msg: () -> Any?) {
        if (KmLogging.isLoggingError)
            errorApi(tag ?: tagName, msg().toString(), err)
    }
